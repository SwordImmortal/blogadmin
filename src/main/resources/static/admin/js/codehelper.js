...
var tools = new Vue(
		{
			...
			methods : {
				...
				getSetMethod : function() {
					...
					String
					newObj = className + " " + objectName + " = new "
							+ className + "();\n";
...
var params = new Vue(
		{
			...
			methods : {
				...
				add : function() {
					var newObject = $.extend(true, {}, initParameter);
				...
				changeAppendSql : function(index) {
					...
					this.parameters[index].appendSql = name ? "AND " + name
							+ " " + matchType
							+ (assignmentType == 1 ? ":" + name : "?") : "";
				},
				...
				reset : function() {
					this.parameters = [];
					this.parameters.push($.extend(true, {}, initParameter));
				},
				joinSql : function() {
					this.sql = doJoinSql();
				}
...
var doJoinSql = function() {
	...
	params.parameters.forEach(function(parameter, index) {
		...
		// 获取变量值
		sql += dataType + " " + name + " = MapUtils.get" + dataType
				+ "(params, \"" + name + "\");";
		// 变量判断是否为空
		...
		likeName = matchType == 'like' ? "Common.ConvertLikeParamer(" + name
				+ ")" : name;
		// 参数赋值
		if (assignmentType == 1) {
	...
	if (frame == "springjdbc" && assignmentType == 1) {
		sql += "this.getNamedParameterJdbcTemplate().queryForList(" + sqlName
				+ ".toString(), parameters);";
	} else if (frame == "springjdbc" && assignmentType == 2) {
		sql += "this.getJdbcTemplate().queryForList(" + sqlName
				+ ".toString(), parameters.toArray());";
...
$(function() {
	$(".entityButton,.checkbox-inline,.radio-inline").hide();
	$("#joinSql").hide();
	$('li').click(function() {
		...
		if (id == "toolsHeader") {
			...
			$("#joinSql").hide();
			$("#tools").show();
			$(".entityButton,.checkbox-inline,.radio-inline").show();
			$(".strButton").hide();
